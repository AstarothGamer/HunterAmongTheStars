...
public class FirstPersonController : MonoBehaviour
{
    ...
    public float leanOffset = 0.5f; // How far the camera moves when leaning
    ...
    void Leaning()
    {
        ...
        if (Input.GetKey(KeyCode.Q)) // Lean left
        {
            ...
            targetCameraPosition += Vector3.left * leanOffset; // Move camera to the left
        }
        else if (Input.GetKey(KeyCode.E)) // Lean right
        {
            targetLean = -leanAngle;
            targetCameraPosition += Vector3.right * leanOffset; // Move camera to the right
        }
        // Smoothly transition to the desired lean angle and camera position
        currentLean = Mathf.Lerp(currentLean, targetLean, Time.deltaTime * leanSpeed);
        CamHolder.transform.localRotation = Quaternion.Euler(CamHolder.transform.localRotation.eulerAngles.x, CamHolder.transform.localRotation.eulerAngles.y, currentLean);
        CamHolder.transform.localPosition = Vector3.Lerp(CamHolder.transform.localPosition, targetCameraPosition, Time.deltaTime * leanSpeed);
    }
    #endregion
}